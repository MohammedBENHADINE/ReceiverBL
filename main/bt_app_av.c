...
/* input control service */
static void input_control_service(void *arg);
/* a2dp event handler */
...
static xTaskHandle s_input_ctrl_task_hdl = NULL;    /* handle for input control service task */
...
void bt_i2s_driver_install(void)
{
    ...
    i2s_config_t i2s_config = {
        ...
        .communication_format = I2S_COMM_FORMAT_STAND_I2S,  /* FMT is grounded, use of I2S , DATA at next LRCK */
        .dma_buf_count = 8,
        .dma_buf_len = 64,
...
static void input_control_service(void* arg)
{
    ...
    uint32_t io_num;
    ESP_LOGI(BT_RC_TG_TAG, "start input control service");
    static uint8_t switchBetweeb = 1;

    for(;;) {
        if(xQueueReceive(gpio_evt_queue, &io_num, portMAX_DELAY)) {
            uint8_t iostate = gpio_get_level(io_num);
            ESP_LOGI(BT_RC_TG_TAG,"GPIO[%d] intr, val: %d\n", io_num, iostate);
            if (VOLUP_IO == io_num )
            {
                uint8_t volume = (s_volume + 5) & 0x7f;
                volume_set_by_local_host(volume);
            }
            if (VOLDOWN_IO == io_num)
            {
                uint8_t volume = (s_volume - 5) & 0x7f;
                volume_set_by_local_host(volume);
            }
...
static void bt_av_hdl_avrc_tg_evt(uint16_t event, void *p_param)
{
    ...
    case ESP_AVRC_TG_CONNECTION_STATE_EVT: {
        ...
        ESP_LOGI(BT_RC_TG_TAG, "AVRC conn_state evt: state %d, [%02x:%02x:%02x:%02x:%02x:%02x]",
                 rc->conn_stat.connected, bda[0], bda[1], bda[2], bda[3], bda[4], bda[5]);
        if (rc->conn_stat.connected) {
            /* create input control service */
            /* TO DO register ISRs */
        } else {
            //vTaskDelete(s_input_ctrl_task_hdl);
            ESP_LOGI(BT_RC_TG_TAG, "Stop input control service");
            /* TO DO unregister ISRs on IOs, delete queue */
        }
    ...
    case ESP_AVRC_TG_REGISTER_NOTIFICATION_EVT: {
        ...
        if (rc->reg_ntf.event_id == ESP_AVRC_RN_VOLUME_CHANGE) {
            ESP_LOGI("MEM_DEBUG", "As_volume_notify set to TRUE");
...
void bt_app_input_ctrl_service(void)
{
    ...
    gpio_set_intr_type(VOLUP_IO, GPIO_INTR_ANYEDGE);
    gpio_set_intr_type(VOLDOWN_IO, GPIO_INTR_ANYEDGE);

    gpio_install_isr_service(ESP_INTR_FLAG_DEFAULT);

    gpio_isr_handler_add(NEXT_IO, gpio_isr_handler, (void*) NEXT_IO);
    //gpio_isr_handler_add(PREVIOUS_IO, gpio_isr_handler, (void*) PREVIOUS_IO);
    gpio_isr_handler_add(STOP_IO, gpio_isr_handler, (void*) STOP_IO);
    //gpio_isr_handler_add(PLAY_PAUSE, gpio_isr_handler, (void*) PLAY_PAUSE);
    gpio_isr_handler_add(VOLUP_IO, gpio_isr_handler, (void*) VOLUP_IO);
    gpio_isr_handler_add(VOLDOWN_IO, gpio_isr_handler, (void*) VOLDOWN_IO);

    xTaskCreate(input_control_service, "inputCtrlTask", 2048, NULL, 5, &s_input_ctrl_task_hdl);
}
